...
void String2Wstring(std::string sStr, std::wstring& wsStr) {
	wchar_t* wStr = NULL;
	int strLen = sU8xU(wStr, (char*)sStr.c_str(), sStr.length());
	wStr = new wchar_t[strLen + 1];
	wmemset(wStr, 0, strLen + 1);

	sU8xU(wStr, (char*)sStr.c_str(), sStr.length());
	wsStr = wStr;
	delete[] wStr;
}
...
std::string Wstring2String(const std::wstring& ws)
{
	...
	wcstombs(_Dest,_Source+1,_Dsize);
               ...
               bool collectComments )
{
   ...
   std::wstring tem;
   String2Wstring(doc, tem);
   doc = Wstring2String(tem);
