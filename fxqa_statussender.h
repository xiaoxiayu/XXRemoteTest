...
namespace fxqa_status {
    ...
    class CHttp {
		static bool bCloseLink;
    public:
        CHttp();
        ~CHttp();

		static void CloseLink();
        static int  RecvData(int conn, std::string& data);
        
        static int  POST(std::string host, int port, std::string request, std::string msg, std::string& recv_msg);
        static int  GET(std::string host, int port, std::string request, std::string& recv_msg);
    };
    ...
    public:
        ...
        int						POST(std::string request, std::string msg, std::string& recv_msg);
        int                     GET(std::string request, std::string& recv_msg);
        void					SetTestInfo(std::string testcase_name, std::string test_name);
        int						SendStartCrash(std::string testCaseName, std::string testName,
                                               std::string sVersion, std::string testfile, int iHeartTime);
        int						SendStartDeath(std::string testCaseName, std::string testName,
    ...
    class CLongTime {
		static bool* pStartFlag;
	public:
		CLongTime() {};
		~CLongTime() {};

		static void Regular(int iTime);
